...
## This DEMO is a 3D example [x,y,z] with constant velocity and with two shots
# 
...
t=fmm.eikonal(vel,xyz=np.array([[0.5,0,0],[0.5,0+0.5,0]],dtype='float32'),ax=[0,0.01,101],ay=[0,0.01,101],az=[0,0.01,101],order=2);
time2shots=t.reshape(101,101,101,2,order='F'); #[x,y,z]
time=time2shots[:,:,:,0]
time2=time2shots[:,:,:,1]
...
plt.title('Shot 1',color='k', fontsize=20)
...
# plt.gca().scatter(0.5,0+0.5,0,s=500,marker='*',color='r')
plt.gca().set_xlim(0,1);
plt.gca().set_ylim(0,1);
plt.gca().set_zlim(0,1);
plt.gca().text(-0.124, 0, -0.66, "b)", fontsize=28, color='k')
plt.title('Shot 2',color='k', fontsize=20)
plt.gca().invert_zaxis()
plt.gca().scatter(0.85,-0.1,-0.5,s=500,marker='*',color='r')
# position for the colorbar
cb = plt.colorbar(C, cax = fig.add_axes([0.37,0.1,0.3,0.02]), format= "%4.2f", orientation='horizontal',label='Traveltime (s)')

plt.savefig('test_pyekfmm_fig2-multishots.png',format='png',dpi=300,bbox_inches='tight', pad_inches=0)
plt.savefig('test_pyekfmm_fig2-multishots.pdf',format='pdf',dpi=300,bbox_inches='tight', pad_inches=0)
